...
const IntroducePage = () => {
  ...
  const autoNextFeature = () => {
    ...
    const currIndexFeature = listFeature.findIndex(item => item.id === featureSelect);
        ...
        <Row>
          <Col lg={12}>
            ...
            <div className={"feature-sum"}>
              {listFeature.map(item => <div key={item.id}
                                            className={`feature ${featureSelect === item.id ? "feature-select" : ""}`}
          ...
          <Col lg={12} className={"feature-demo"}>
